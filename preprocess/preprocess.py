...
dataset_dict = {'openkp':[('EvalPublic', 'eval'), ('Dev', 'dev'), ('Train', 'train')], 
                ...
                'inspec':[('testing', 'eval')],
                'nus':[('testing', 'eval')],
                'krapivin':[('testing', 'eval')],
                'semeval':[('testing', 'eval')]}
...
def add_preprocess_opts(parser):
    ...
    parser.add_argument("--dataset_class", type=str, choices=['openkp', 'kp20k','inspec', 'nus', 'krapivin', 'semeval'],
...
def nus_loader(mode, source_dataset_dir,
                 src_fields=['title', 'abstract'],
                 trg_fields=['keyword'], trg_delimiter=';'):
    ''' load source nus dataset :'title', 'abstract', 'keyword'
    return : tuple : src_string, trg_string'''

    logger.info("start loading %s data ..." % mode)
    source_path = os.path.join(source_dataset_dir, 'nus_%s.json' % mode)
...
def inspec_loader(mode, source_dataset_dir,
                 src_fields=['title', 'abstract'],
                 trg_fields=['keyword'], trg_delimiter=';'):
    ''' load source inspec dataset :'title', 'abstract', 'keyword'
    return : tuple : src_string, trg_string'''

    logger.info("start loading %s data ..." % mode)
    source_path = os.path.join(source_dataset_dir, 'inspec_%s.json' % mode)
...
def krapivin_loader(mode, source_dataset_dir,
                 src_fields=['title', 'abstract'],
                 trg_fields=['keyword'], trg_delimiter=';'):
    ''' load source nus dataset :'title', 'abstract', 'keyword'
    return : tuple : src_string, trg_string'''
    ...
    logger.info("start loading %s data ..." % mode)
    source_path = os.path.join(source_dataset_dir, 'krapivin_%s.json' % mode)
...
def semeval_loader(mode, source_dataset_dir,
                 src_fields=['title', 'abstract'],
                 trg_fields=['keyword'], trg_delimiter=';'):
    ''' load source nus dataset :'title', 'abstract', 'keyword'
    return : tuple : src_string, trg_string'''
    ...
    logger.info("start loading %s data ..." % mode)
    source_path = os.path.join(source_dataset_dir, 'semeval_%s.json' % mode)
...
def inspec_refactor(src_trgs_pairs, mode, valid_check=True):
    logger.info("start refactor kp20k %s data ..." % mode)
...
def nus_refactor(src_trgs_pairs, mode, valid_check=True):
    logger.info("start refactor kp20k %s data ..." % mode)
...
def semeval_refactor(src_trgs_pairs, mode, valid_check=True):
    logger.info("start refactor kp20k %s data ..." % mode)
...
def krapivin_refactor(src_trgs_pairs, mode, valid_check=True):
    logger.info("start refactor krapivin %s data ..." % mode)
...
def main_preprocess(opt, input_mode, save_mode):
    ...
    if opt.dataset_class == 'inspec' and save_mode != 'train':
        save_ground_truths(feed_data, gt_filename, kp_key='keyphrases')
    if opt.dataset_class == 'nus' and save_mode != 'train':
        save_ground_truths(feed_data, gt_filename, kp_key='keyphrases')
    if opt.dataset_class == 'krapivin' and save_mode != 'train':
        save_ground_truths(feed_data, gt_filename, kp_key='keyphrases')
    if opt.dataset_class == 'semeval' and save_mode != 'train':
        save_ground_truths(feed_data, gt_filename, kp_key='keyphrases')
...
if __name__ == "__main__":
    ...
    # preprocess selector
    data_loader = {'openkp':openkp_loader, 'kp20k':kp20k_loader, 'inspec':inspec_loader, 'nus':nus_loader, 'krapivin':krapivin_loader, 'semeval':semeval_loader}
    data_refactor = {'openkp':openkp_refactor, 'kp20k':kp20k_refactor, 'inspec':inspec_refactor, 'nus':nus_refactor, 'krapivin':krapivin_refactor, 'semeval':semeval_refactor}
    absent_filter = {'openkp':filter_openkp_absent, 'kp20k':filter_kp20k_absent, 'inspec':filter_kp20k_absent, 'nus':filter_kp20k_absent, 'krapivin':filter_kp20k_absent, 'semeval':filter_kp20k_absent}
