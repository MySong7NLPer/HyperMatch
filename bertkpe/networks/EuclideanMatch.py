...
class RobertaForCnnGramRanking(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.c = 1
        self.euclidean_linear = nn.Linear(cnn_output_size, 1)
...
class RobertaForTFRanking(RobertaForCnnGramRanking):
    def forward(self, input_ids, attention_mask, valid_ids, active_mask, valid_output, labels=None):
        ...
        # --------------------------------------------------------------------------------
        # Importance Score
        classifier_scores1 = -torch.sum(torch.square(ngrams_outputs - mean_document.expand_as(ngrams_outputs))) / math.sqrt(self.rank_value)
        classifier_scores2 = self.euclidean_linear(ngrams_outputs.view(-1, self.rank_value)).view(batch_size, -1)
        classifier_scores = 0.5 * classifier_scores1 + 0.5 * classifier_scores2
        classifier_scores = classifier_scores.unsqueeze(1).expand(active_mask.size())
